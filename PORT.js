...
export function handlePortMainFilteredData(receivedData, filtersConfig, elementId) {
  ...
  [mvarIR, mvarCS, mvarTOT] = MVaRData.map(item => item.VaR);
  [esIR, esCS, esTOT] = MVaRData.map(item => item.ES);
  ...
  if (portDataContainer && portData) {
    ...
    const formMvarTOT = (parseFloat(mvarTOT) * 100).toFixed(2);
    const formMvarIR = (parseFloat(mvarIR) * 100).toFixed(2);
    const formMvarCS = (parseFloat(mvarCS) * 100).toFixed(2);
