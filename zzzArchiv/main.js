    ...
    if (err) {
      ...
      if (rows.length === 0) {
        ...
        const formatColumns = ['RATES','NOTIONAL', 'NAV','COUPON', 'START_DATE', 'MATURITY', 'TRADE_DATE', 
                                'C_SPREAD', 'clean_price', 'GEARING', 'FLOOR', 'CAP', 'SPREADS',
            ...
            if (column in firstRow) {
              switch (column) {
                ...
                case 'C_SPREAD': 
                    ...
                    // formattedRow[column] = `<span style="color: ${color}; font-weight: bold; display: flex; justify-content: center;">${formattedValue}</span>`;
                    // formattedRow[column] =(row[column]).toFixed(0);
                    const color = 'rgb(70, 192, 230)';
                    const formattedValue = parseFloat(row[column]).toFixed(0);
...
getAllTableNames((err, receivedTableNames) => {
  if (err) {
    ...
    const excludedTableNames = ['sqlite_sequence', 'Instruments', 'PDMain', 'MVaRMainDist' , 'sortedLossesMain', 'sortedLossesIndicesMain']; // Add any other names you want to exclude
...
// Handle selected issuer change
ipcMain.on('selected-issuer-changed', (event, selectedIssuer) => {
  handlePortMainData(initialReceivedData, selectedIssuer);
});
