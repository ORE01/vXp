...
export function handleFWDData(receivedData, applyCubicSpline) { 
  ...
  // 👉 Preview informieren, dass sich Charts + Tabelle geändert haben
  notifyRiskPreview();
}
...
export function handleSwapForwardCurve(receivedData) {
  ...
  // ⬇️ NEU: Risk-Preview aktualisieren
  try {
    document.dispatchEvent(new CustomEvent('risk:refresh-thumbnails'));
  } catch (e) {
    console.warn('Failed to dispatch risk:refresh-thumbnails after handleSwapForwardCurve', e);
  }
}
