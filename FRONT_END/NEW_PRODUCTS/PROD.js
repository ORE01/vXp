import processData, { filterColumnsInData } from '../../MODAL_HELPER/dataProcessor.js';
...
export function handleProdData(filtersConfig) {
  ...
  // Delegierter Klick-Listener für PROD_ID-Buttons (einmalig)
...
function checkCSSzenarioFlag(filteredProdData) {
  ...
  if (!csWarningContainer || !csWarningLight) {
    console.error("⚠️ 'csWarningContainer' oder 'csWarning' nicht gefunden!");
    return;
  }
...
/**
 * Bringt ein ProdAll-Row-Objekt in die gewünschte Feld-Reihenfolge
...
export function buildOrderedFieldsForModal(row) {
  ...
  // 2) Alle übrigen Felder hinten anhängen
