...
export function getRiskReportOptions() {
  ...
  return {
    ...
    sections: {
      // alt:
// portfolioBreakdown: chk('rr-portfolioBreakdown', REPORT_DEFAULTS.sections.portfolioBreakdown),

// neu:
portfolioBreakdown: (() => {
  const def = REPORT_DEFAULTS.sections.portfolioBreakdown || { enabled: true, issuer: true, product: true, general: true };

  const uiEnabled = document.getElementById('rr-portfolioBreakdown'); // Haupttoggle (falls vorhanden)
  const enabled = uiEnabled ? !!uiEnabled.checked : (def.enabled !== false);

  // Gruppen-Schalter NUR aus den Report Options lesen
