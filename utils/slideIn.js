...
function initContainer(container) {
  ...
  container.addEventListener('click', (e) => {
    ...
    const isBackdrop =
      ...
      const isSub = triggerBtn.classList.contains('section-trigger--sub');

      // 🔹 Nur Haupt-Trigger klappen ihre Sub-Trigger auf/zu
      if (!isSub) {
        toggleSubSections(triggerBtn);
      }
...
if (document.readyState === 'loading') {
  ...
  function toggleSubSections(mainButton) {
    const mainSection = mainButton.closest('.chart-section');
    const isOpen = mainButton.classList.toggle('is-open');

    // Alle nachfolgenden .chart-section--sub ein-/ausblenden,
    // bis der nächste Haupt-Block (.chart-section ohne --sub) kommt
    let sibling = mainSection.nextElementSibling;
    while (sibling && sibling.classList.contains('chart-section--sub')) {
      sibling.style.display = isOpen ? 'block' : 'none';
