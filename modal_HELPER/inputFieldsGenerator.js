...
// 'today' → heutiges Datum (lokal), 'YYYY-MM-DD' → Date, sonst null
function parseFlexibleStartDate(raw) {
  const v = String(raw || '').trim().toLowerCase();
  if (!v) return null;
  if (v === 'today' || v === 'heute' || v === 'now') {
    const now = new Date();
    // Uhrzeit egal → auf lokalen Tag normalisieren
...
function parseISO(v) {
  ...
  // Validierung (JS korrigiert sonst still)
  if (dt.getFullYear() !== y || dt.getMonth() !== mo || dt.getDate() !== d) return null;
  return dt;
}

// '10y', '18m', '30d', '2w' → Dauer-Objekt; sonst null
function parseDuration(raw) {
  const m = /^([+-]?\d+)\s*(y|yr|yrs|year|years|m|mo|mon|month|months|w|wk|wks|week|weeks|d|day|days)$/i.exec(String(raw || '').trim());
  if (!m) return null;
  const n = parseInt(m[1], 10);
  const u = m[2].toLowerCase();
  if (['y','yr','yrs','year','years'].includes(u)) return { years: n };
  if (['m','mo','mon','month','months'].includes(u)) return { months: n };
  if (['w','wk','wks','week','weeks'].includes(u)) return { days: n * 7 };
  if (['d','day','days'].includes(u)) return { days: n };
  return null;
}

function isRelativeDuration(raw) {
  return !!parseDuration(raw);
}

function addDuration(baseDate, dur) {
  ...
  if (dur.years) {
    res = new Date(res.getFullYear() + dur.years, res.getMonth(), res.getDate());
  }
  if (dur.months) {
    // Monate addieren → JS rollt korrekt (31 → Feb etc. wird angepasst)
    const ny = res.getFullYear();
    const nm = res.getMonth() + dur.months;
    // Tag versuchen zu halten; JS korrigiert, falls nicht existiert (z.B. 31. auf Feb)
    res = new Date(ny, nm, res.getDate());
  }
  if (dur.days) {
    res = new Date(res.getFullYear(), res.getMonth(), res.getDate() + dur.days);
  }
  return res;
}

// MATURITY: absolut (ISO) oder relativ (Dauer) zum START_DATE
function parseFlexibleMaturity(raw, startDate) {
  if (!raw) return null;
  // 1) ISO?
  ...
  if (dur && startDate) return addDuration(startDate, dur);
  return null;
}
