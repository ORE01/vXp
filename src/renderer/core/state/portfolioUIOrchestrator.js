...
function renderOffersTable(data, opts = {}) {
  ...
  // B2) ENRICHED rows:
  // âœ… If filter-engine passed rows in `data`, use them (this is the missing link!)
  ...
  renderOffersSensTables({ baseRows, enriched });             // -> IRSens/CSSens
        ...
        function renderOffersRawDealsTable({ offerName, baseRows } = {}) {
          ...
          // - restoreDropdown:false verhindert, dass irgendwelche Dropdowns "zurÃ¼ck-geklickt" werden
