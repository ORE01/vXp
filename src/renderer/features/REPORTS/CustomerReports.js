...
export function handleCustomerReportsView() {
  ...
  if (!container) {
    console.warn('âš ï¸ Container #customerReportsContainer nicht gefunden.');
    return;
  }
  ...
  // State zurÃ¼ckschreiben (wie bei dir Ã¼blich)
  ...
  ensureRendered(async () => {
    const reload = async () => {
      ...
      // Alternativ: hier fetch triggern, falls du das so gelÃ¶st hast.
    };
...
function extractArg(a, b) {
  // wenn once(handler) mit (event, payload) kommt â†’ b ist payload
  // wenn once(handler) mit (payload) kommt â†’ a ist payload
