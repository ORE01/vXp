...
// Root nur im Panel suchen (fÃ¤llt zurÃ¼ck auf altes Modal, falls vorhanden)
const liquRoot =
...
function renderLiquidityChart(data) {
  ...
  const canvas = document.getElementById('liquChart');
  if (!canvas || !canvas.isConnected) return;
  if (canvas.offsetParent === null) return; // hidden -> nicht rendern
  ...
  if (years.length === 0 || catsSorted.length === 0) {
    ...
    canvas.parentElement?.removeAttribute('data-empty');
  }
  ...
  // ✅ nur destroyen, wenn Canvas noch da ist (ist durch guard gesichert)
  if (liquChart) liquChart.destroy();
  ...
  liquChart = createBarChart(
    ...
    'bar',
    'x'
    // (dein options-Objekt wird aktuell von createBarChart ignoriert – lassen wir hier bewusst weg)
  );
}
