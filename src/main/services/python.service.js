...
function startPythonScript({
  ...
  return new Promise((resolve, reject) => {
    ...
    let proc;
    try {
      // optional: windowless on Windows, falls nötig
    ...
    proc.on('close', (code) => {
      ...
      // ✅ KRITISCH: Restbuffer flushen (sonst fehlt RESULT oft!)
      if (stdoutBuffer.trim()) {
        const rest = stdoutBuffer.trim();
        // auch hier noch progress-json versuchen
        try {
      ...
      if (stderrBuffer.trim()) {
        ...
        // nicht zwingend, aber hilfreich fürs Debug
        scriptOutput += '\n[STDERR]\n' + restErr + '\n';
